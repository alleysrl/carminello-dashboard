  ...
  function vOrdini() {
    ...
    el("view").querySelectorAll("[data-det]").forEach(b => b.onclick = () => {
      ...
      modal(`<h2>Ordine ${o.numero}</h2><dl class="kv"><dt>Data</dt><dd>${dateL(o.created_at)}</dd><dt>Cliente</dt><dd>${esc(nome(Object.keys(c).length ? c : a))} · ${esc(c.email || "")} · ${esc(a.telefono || c.telefono || "")}</dd>
        ...
        ${o.agente_id ? `<dt>Agente</dt><dd><a href="#/agente/${o.agente_id}">${esc(nomeAg(D.agenteBy[o.agente_id]))}</a> · provvigione ${o.provvigione_pct}% = <b>${money(o.provvigione)}</b>${o.stato === "annullato" ? " (ordine annullato: nessuna provvigione)" : o.provvigione_liquidata_il ? " (liquidata)" : o.pagato ? " (maturata)" : " (matura al pagamento)"}</dd>` : ""}</dl>
